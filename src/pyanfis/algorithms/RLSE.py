...
class RLSE(torch.nn.Module):
    ...
    def __init__(self, n_vars, initial_gamma=1000.):
        super().__init__()
        self.S = torch.eye(n_vars, dtype=torch.float32, requires_grad=False) * initial_gamma
        self.theta = torch.zeros((n_vars, 1), dtype=torch.float32)
        self.gamma = 1000.0
    ...
    def forward(self, A, B):
        ...
        for ba in range(batch):
            for i in range(row):
                ...
                self.S.add_(- (torch.matmul(torch.matmul(torch.matmul(self.S, a.T), a), self.S)) / (1 + torch.matmul(torch.matmul(a, self.S), a.T)))
                self.theta.add_(torch.matmul(self.S, torch.matmul(a.T, (b - torch.matmul(a, self.theta)))))