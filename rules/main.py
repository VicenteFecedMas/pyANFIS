...
class Rules(torch.nn.Module):
    ...
    def relate_fuzzy_numbers(self, fuzzy_numbers_matrix):
        ...
        rules_per_universe = torch.empty((fuzzy_numbers_matrix.size(0), fuzzy_numbers_matrix.size(1), self.active_rules.size(0), fuzzy_numbers_matrix.size(2)))
    ...
    def forward(self, x, epoch):
        ...
        return x.reshape(x.size(0), x.size(1),x.size(2)), [self.binarice(rule) for rule in self.active_rules]
